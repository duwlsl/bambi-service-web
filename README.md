...
- 팀 프로젝트 당시의 애플리케이션 코드는 그대로 두고, **종료 후의 개인 개선은 별도 브랜치·커밋으로만** 올립니다.
...
| Vitest + Testing Library + MSW (jsdom) | 화면 렌더와 네트워크 계약 (상세 화면, 피드 오류·Empty, 모달 포커스, 중복 요청 회귀, 알림 배경 폴링) | 9파일 **50 케이스 통과** |
...
**2026-08-28 — 알림 배경 폴링 정합성** (`fix/notification-polling-consistency`)

- 알림 조회를 `useAsyncData` + `setInterval(refetch, 30_000)` 조합에서 stale-while-revalidate 폴링 훅
  [hooks/use-polled-data.ts](hooks/use-polled-data.ts)로 이관했습니다. 폴링 훅 자체는 수정하지 않았습니다.
...
- 훅의 외부 반환 계약을 유지해 소비 컴포넌트(헤더 드롭다운 · `/notifications`)는 수정하지 않았고,
  MSW로 요청 수를 세는 폴링 테스트 10건을 추가했습니다. 그중 5건은 이전 구현에서 실패하는 것을 확인해 회귀 테스트로서의 유효성을 검증했습니다.
