/**
 * 브리핑 본문 mock — 목업 report-detail.html .md 블록 1:1 (mdtag 라벨은 display:none → 미포함).
 *
 * ★ 실제 API 교체 지점: 이 컴포넌트 전체가 마크다운 렌더러 호출로 대체된다.
 *   API가 content_md(마크다운 원문)를 내려주면 렌더 결과를 .md-viewer 로 감싼다.
 *   (lib/mock/report.ts 상단 주석 참조)
 */
export function ReportBodyMock() {
  return (
    <div className="md-viewer">
      ...
      <ul>
        ...
        <li>
          회원님의 <b>환전 예정(이달 말)</b> 시점과 관련해 확인할 만한 변동입니다.
        </li>
      </ul>
      ...
      <h2>왜 지금 중요한가</h2>
      <p>
        회원님이 등록한 환전 예정 시점(이달 말)과 목표 환율을 기준으로 보면 현재 환율은 등록한
        범위에 근접해 있습니다. 다만 오늘 밤 CPI 수치가 예상을 웃돌 경우 <b>단기 반등 변수</b>가
        있어, 이후 흐름은 달라질 수 있습니다.
      </p>
      ...
      <p>
        임계값 <code>threshold: 0.5</code>가 너무 자주 걸린다면 <a aria-disabled="true">LLM Wiki</a>
        에서 조건을 직접 조정할 수 있어요.
      </p>
      ...
      <div className="mt-[26px] flex items-center gap-[9px] border-t border-border pt-[18px]">
        <span className="mr-auto text-[13px] font-semibold text-ink-mid">
          이 주제, 계속 받아볼까요?
        </span>
        <button
          type="button"
          aria-disabled="true"
          className="inline-flex items-center justify-center gap-1.5 rounded-lg border border-border bg-card px-3 py-[7px] text-[12.5px] font-semibold whitespace-nowrap text-foreground"
        >
          👍 이 주제 더 보기
        </button>
        <button
          type="button"
          aria-disabled="true"
          className="inline-flex items-center justify-center gap-1.5 rounded-lg border border-border bg-transparent px-3 py-[7px] text-[12.5px] font-semibold whitespace-nowrap text-ink-mid"
        >
          그만 보기
        </button>
      </div>
