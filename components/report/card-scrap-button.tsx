 ...
 *
 * **PUBLIC 카드 또는 내 소유 카드에서만** 렌더된다(노출 판정은 호출부). 백엔드는 스크랩 대상을
 * "PUBLIC 이거나 내 카드" 로 둔다(service-api #85 — 그전에는 PUBLIC 뿐이었다). 남의 비공개 카드에
 * 두면 확실한 404 를 부른다. 좋아요와 같은 정책으로 **작성자 본인도 담을 수 있다** — PUBLIC 에서는
 * 본인 여부로 숨기거나 막지 않고, PRIVATE 에서는 본인 카드일 때만 노출된다.
 *
