...
import { parseFollowTargetPath, resolveNotificationTarget } from "@/lib/notifications/resolve-notification-target";
...
const FOLLOW_TARGET_ERROR_MESSAGE = "연결된 프로필을 찾지 못했어요.\n잠시 후 다시 시도해 주세요.";
 ...
 *
 * REPORT_READY 흐름(변경 없음): 행 pending lock → targetPath 검증 + feed 기반 resolve
 ...
 *
 * FOLLOW 흐름(REPORT_READY와 순서가 다르다 — 의도적): targetPath 는 이미 올바른 프로필 경로라
 * feed 재해석이 필요 없으므로, 형식만 검증 → 미읽음이면 read PUT을 먼저 기다렸다가 성공 시
 * onReadConfirmed() 호출 → 그 다음 이동한다. read PUT이 실패해도 프로필 자체는 유효하므로 이동은
 * 진행하되(사용자를 막지 않는다) unread 는 그대로 둔다(다음 조회가 서버 진실과 맞춘다).
 */
export function useNotificationOpen(onReadConfirmed: () => void) {
  ...
  const openNotification = useCallback(
    async (notification: NotificationDto): Promise<boolean> => {
      ...
      if (notification.type === FOLLOW_TYPE) {
        const path = parseFollowTargetPath(notification.targetPath);
        if (path === null) {
          setPendingId(null);
          setError({ id: notification.id, message: FOLLOW_TARGET_ERROR_MESSAGE });
          return false;
        }
        ...
        if (!notification.read) {
          try {
            ...
            // 읽음 반영 실패 — 프로필은 이미 유효해 이동은 진행된다. unread 를 그대로 두고
