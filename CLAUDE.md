...
### ⚡ 2026-08-05 범위 변경 — /wiki 목업 정렬 (우석 결정 "지금꺼 빼버리고 목업대로")

- **/wiki 상단을 목업 wiki.html 구조로 재구성**: ① AI가 이해한 지금의 나(.mind — score 강도 바,
  행 클릭 = 하단 자료 필터) ② AI가 최근 발견한 관심사(.found — INFERRED 태그 중 내 관심사에 없는 것,
  **＋추가 = `POST /api/interests`**) ③ **내 관심사**(.wcard — `GET/DELETE /api/interests`, USER 목록 관리,
  이름 일치하는 INFERRED 태그가 있으면 "LLM 추론 일치" + 신뢰도 % + evidence 근거 병기) ④ 내가 저장한
  자료(기존 유지 — 07-27 정보구조가 목업보다 우선). 기존 "AI가 이해한 관심사" 태그 카드 섹션(wiki-interests.tsx)은 삭제.
- **백엔드 없어서 만들지 않은 목업 요소**(동작하지 않는 UI 금지): 발견 후보의 "무시" · 고정됨 ·
  잠시 쉬는 중(활성/비활성) · 수정하기(rename) 모달 · rail 의 활성/비활성·제외한 주제 통계.
...
`search.html` · `notifications.html` · `profile-self.html` · `profile-user.html` · `saved.html` · 랜딩(`landing/landing-desktop.html`)

> `settings.html` 은 2026-07-28 우석 승인으로 이 목록에서 빠졌다 (아래 범위 변경 참조).
> `onboarding.html` 은 2026-07-30 구현 승인으로 이 목록에서 빠졌다 (위 범위 변경 참조).
> **소셜 로그인(Google)** 은 2026-08-10 P2(MVP 제외)로 재분류되어 이 P1 목록에서 제외했다. 동작하지 않는 Google 버튼과 로그인 화면 `.auth-or` 구분선도 제거했다 (§7 충돌표 #2 · §15).
