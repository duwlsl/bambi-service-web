...
> **소셜 로그인(Google)** 은 2026-08-10 「제공하지 않음」 확정으로 **미구현을 넘어 목업의 버튼까지 화면에서 걷어냈다** —
> 로그인·가입 화면의 Google 버튼과 로그인 화면 `.auth-or`(또는) 구분선을 제거했다 (§7 충돌표 #2 · §15).
> 이 목록의 다른 항목은 "안 만들었을 뿐"이지만 이 항목은 "만들었다가 지웠다"라 상태가 다르다.
