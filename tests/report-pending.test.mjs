...
test("타임존을 고정해 실행 환경 기본 시간대와 무관하게 같은 값을 만든다", () => {
  ...
  assert.deepEqual([...seen], ["8월 11일 18:30 생성"]);
});

test("자정 직전·직후도 KST 날짜로 끊는다", () => {
  // 2026-08-11T15:00:00Z = KST 2026-08-12 00:00 → 날짜가 하루 넘어가야 한다.
  assert.equal(formatPendingCreatedAt("2026-08-11T15:00:00Z"), "8월 12일 00:00 생성");
  assert.equal(formatPendingCreatedAt("2026-08-11T14:59:00Z"), "8월 11일 23:59 생성");
});
...
test("빈 문자열 결과에는 '생성' 접미사조차 남지 않는다", () => {
  // "  생성" 처럼 값 없이 접미사만 남으면 화면에 의미 없는 꼬리가 붙는다.
  assert.equal(formatPendingCreatedAt(null).includes("생성"), false);
  assert.equal(formatPendingCreatedAt("not-a-date").includes("생성"), false);
});
...
test("Pending DTO 의 createdAt 을 그대로 넣으면 표기가 만들어진다", () => {
  ...
  assert.equal(formatPendingCreatedAt(dto.createdAt), "8월 10일 09:00 생성");
});
